...
#############################################################################################
#                    THIS SCRIPT TAKES A STDIN OR A FILE INPUT:                             #
#                                                                                           #
#    1. IT SCORES THE EMOTION OF THE INPUT (in ../outputs/fromfile or ../outputs/fromstdin) #  
#    2. IT BACKTRANSLATES THE INPUT                                                         #
#    3. IT RETURNS THE BEST SCORING PARAPHRASE FOR EACH TARGET EMOTION                      #
#      (in ../outputs/fromfile or ../outputs/fromstdin)                                     #
#                                                                                           #
#    Note: THE NAME OF THE FILE HAS TO CONTAIN THE NAME OF THE CORPUS IT COMES FROM         #
#                BECAUSE THIS WILL DETERMINE THE EMOTION LABELS AND THE CLASSIFIER          #
#############################################################################################
...
import emotion_module.classify_file as css
...
class Paraphrase(object):    
    ...
    '''
    def __init__(self,sentence="",score=0,EmoScore=0,allEmoScores=0,LMscore=0,sourceLanguage="",targetLanguage="",targetEmotion="",sentenceID=""):
        self._sentence=sentence
        self._score=score
        self._EmoScore=EmoScore
        self._allEmoScores=allEmoScores #this serves just when goal= Restore_Overgenerate
        self._LMscore=LMscore
...
def scoreInput(originalEmo,sentenceID, sentence:str,trainingData):
    ...
    nameFILE=path_to_Output+'scored_'+trainingData
    ...
    with open(nameFILE,'a') as scoredINPUT:
        ...
        print('INPUT EMO SCORE')
        print(input_emotion_score)
    ...
    print('{} : {} : STATUS : Scored original sentence {}.'.format(script, datetime.datetime.now(), str(sentenceID)))
...
def scoreParaphrase(paraphrases:list,goal:str):
    ...
    print('{} : {} : STATUS: Paraphrases obtained by forward-backward translation are being scored by the Emotion Model.'.format(script, datetime.datetime.now()))
    for instance in paraphrases:
        
        p=instance._sentence

        #get a list of emo scores, in this order for ISEAR: Guilt, Joy, Fear, Anger, Sadness, Disgust, Shame
        emoscore=css.makePrediction(p,EC)
        scores=[round(x,3) for x in list(emoscore[1].values())]
                
        #add Emo score (vector)
        instance._EmoScore=scores                        
        scoredParaphrases.append(instance)      

    return scoredParaphrases

'''8. SELECT THE N-BEST PARAPHRASES (n-best=highest score for target emotion)'''
def get_best_paraphrases(scoredParaphrases:list, numTopParaphrases:int,targetEmotion:list,originalEmo,originalEmoscore,ORIGINALemoscores,goal):
    selected=list()
    i=open('restore_overgenerate.txt','a')
    i.write('Original Emo'+originalEmo+' ')
    i.write(str(originalEmoscore)+'\n')
    i.write('these are the original scores '+str(ORIGINALemoscores))
    #select best for every target emotion
    for emo in targetEmotion:
        sort_paraphrases=list()
        emo=emo.strip()
        i.write('\ntarget emo {}'.format(emo))
        ...
        for p in scoredParaphrases:
            ...
            i.write('\nThese scores {}\n'.format(str(scores)))
            i.write('are for the following sentence\n')
...
def translateAndscore(sourceLanguage:str, targetLanguages:str, numbForwtransl:int, numbBacktransl:int, trainingData:str, numTopParaphrases: int, decoding:str, goal: str):
    ...
    # Save output here
    outputFile=open(path_to_Output+'paraphrases_'+trainingData,'w')
